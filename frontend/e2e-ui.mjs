...
const run = async () => {
  ...
  try {
    ...
    await evaluate(send, 'localStorage.clear(); sessionStorage.clear();');
    ...
    const resumePath = path.resolve('../backend/samples/resume.pdf');
    await send('DOM.setFileInputFiles', {
      nodeId: inputNode.nodeId,
      files: [resumePath],
    });
    await waitForText(send, 'resume.pdf', undefined, consoleErrors);
