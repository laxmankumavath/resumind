...
const UploadResume = () => {
  ...
  const handleFile = (selectedFile) => {
    ...
    const isPdfOrDocx = name.endsWith('.pdf') || name.endsWith('.docx') || name.endsWith('.doc');

    if (!isPdfOrDocx && !validMimes.includes(selectedFile.type)) {
  ...
  const onUpload = async () => {
    ...
    try {
      ...
      const resumeId = response?.data?._id || response?._id;
      if (resumeId) {
        navigate(`/dashboard/analysis/${resumeId}`);
      } else {
        navigate('/dashboard');
      }
    } catch (error) {
